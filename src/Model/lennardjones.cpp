...
void LennardJones::calculateForces(System &system)  //object system is passed by reference (allows changing)
{
    ...
    for(int current_index=0; current_index<system.num_atoms(); current_index++){
        ...
        for(int other_index=current_index+1;other_index<system.num_atoms();other_index++){   //+1 to avoid double counting
            Atom *other_atom = system.atoms(other_index);

            //distance and vector btw objects dx
            vec2 displacement(0.,0.);
            displacement = current_atom->position - other_atom->position;
            ...
            current_atom->force += total_force_over_r*displacement;  //all arithmetic operators are overloaded and work on vec2 objects
            other_atom->force -= total_force_over_r*displacement;
        ...
        //std::cout << "force from ext potential" << system.extPotential.getForcefromPotential(current_atom->position, skin_cutoff_sqrd) <<std::endl;
         ...
         for (int current_index=0; current_index<system.num_atoms(); current_index++){
             ...
             if(current_atom->position[0] < l_skin_bndry){
                 ...
                 for (int ghost_index=0; ghost_index < 2*num_from_left; ghost_index+=2) {
                     ...
                     double sigma_over_radius = m_sigma/radius;
                     double total_force_over_r = 24.*(2.0*pow(radius,-14.)-pow(radius,-8.));                     
                     current_atom->force += total_force_over_r*displacement;
                 ...
                 for (int ghost_index=0; ghost_index < 2*num_from_right; ghost_index+=2) {
                     ...
                     */
                     ...
                     vec2 displacement(0.,0.);
                     for(int j=0;j<2;j++){
                         displacement[j] = current_atom->position[j] - from_right[ghost_index + j];
                         //displacement[j] = Atomsto_right[current_index]->position[j] - from_right[ghost_index + j];
                     }
                     ...
                     double sigma_over_radius = m_sigma/radius;
                     double total_force_over_r = 24.*(2.0*pow(radius,-14.)-pow(radius,-8.)); 
                     current_atom->force += total_force_over_r*displacement; //i.e. Fx = (F/r)*x
                     //Atomsto_right[current_index]->force += total_force_over_r*displacement; //i.e. Fx = (F/r)*x

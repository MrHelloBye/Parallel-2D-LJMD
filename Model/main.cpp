...
//Authors: Timofey Golubev, Xukun Xiang based on the code found at https://github.com/andeplane/molecular-dynamics-fys3150.
...
int main(int argc, char **argv)
{
   int my_id, nproc;
    ...
    if( my_id == 0 ) {
        ...
        subsystemSize.set(10,10); //set SUBsystem size
        subsystemOrigin.set(0,0);

        cout << "I compute " <<subsystemSize <<"unit cell subsystem with origin at" << subsystemOrigin << endl;
     }
     else if( my_id == 1 ) {

        /* do some work as process 1 */
       subsystemSize.set(10,10);
       subsystemOrigin.set(10,0);
       cout << "I compute " <<subsystemSize<<"unit cell subsystem with origin at" << subsystemOrigin <<  endl;
...
//         velocities.close();
    ...
    /*
            ...
            */
            ...
            //have only 1 processor write to file (otherwise can't get correct xyz format file)
            if( my_id == 0 ) {
                if(timestep % 100 ==0){
            ...
            if( my_id == 1 ) {
                if(timestep % 100 ==0){
                    //save atom coordinates only periodically to save CPU and file size
                     movie2.saveState(system, statisticsSampler);  //calls saveState fnc in io.cpp which saves the state to the movie.xyz file
                }
            }
